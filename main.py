"""Aggregates Virginia-located listings from every taildraggers.com
...
ones whose location ends in ", VA".
...
Vermont repos' approach, with the state filter swapped to "VA".
"""
...
PAGE_TITLE = "Taildragger Ads in Virginia"
STATE_ABBR = "VA"
...
_VIRGINIA_LOCATION_RE = re.compile(r",\s*VA\s*$", re.IGNORECASE)


def _is_virginia(location: str) -> bool:
    return bool(_VIRGINIA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        virginia_rows = [row for row in rows if _is_virginia(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(virginia_rows)} in Virginia")
        all_listings.extend(virginia_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Virginia listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Virginia listings: {len(listings)}")
