...
Daily aggregator of Virginia-located taildragger classified listings,
...
[Vermont](https://github.com/taildraggers/vermont) repos' approach, with the
state filter swapped to "VA".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, VA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Virginia"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/virginia/`
...
```html
<iframe
  src="https://taildraggers.github.io/virginia/"
  title="Taildragger Ads in Virginia"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_VIRGINIA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
